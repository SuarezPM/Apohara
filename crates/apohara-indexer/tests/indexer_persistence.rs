...
/// This test uses a temporary directory to avoid polluting ~/.apohara
#[test]
...
fn test_indexer_persistence() {
    ...
    // Set environment variable to use temp directory for db
    // Note: The current Indexer uses ~/.apohara hardcoded, so we test the DB directly
    // For full integration test, we'd need to modify Indexer to accept custom path
    
    // For now, test the Indexer creation (which will use ~/.apohara)
    // In a production test, we'd mock the path or add configuration
    let indexer = apohara_indexer::Indexer::new();
    
    // If model isn't available, we can't fully test - but code should compile
    if indexer.is_err() {
        // Model not available - likely needs HF token or download
        println!("Model not available - skipping full integration test");
        return;
    }
    ...
    match result {
        Ok(_id) => {
            ...
            // Note: Can't easily test persistence without custom path support
        }
        Err(e) => {

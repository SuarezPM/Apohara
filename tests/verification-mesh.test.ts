...
import { VerificationMesh, type MeshExecutionOptions } from "../src/core/verification-mesh";
import type { IndexerClient, FileSignaturesResponse } from "../src/core/indexer-client";
...
describe("VerificationMesh", () => {
  ...
  describe("Basic execution", () => {
    ...
    it("should run verification mesh for high complexity tasks", async () => {
      ...
      mockRouteTaskWithFallback.mockResolvedValueOnce({
        provider: "groq",
        response: { content: '{"verdict": "A", "reasoning": "Better implementation"}' },
      });
  ...
  describe("Context compression with IndexerClient", () => {
    ...
    it("should inject IndexerClient via constructor", async () => {
      ...
      mockRouteTaskWithFallback.mockResolvedValueOnce({
        provider: "groq",
        response: { content: '{"verdict": "A", "reasoning": "Cleaner implementation"}' },
      });
...
+export const foo = 2;`,
    ...
    it("should fallback silently when indexer fails", async () => {
      ...
      const failingIndexer = {
        getFileSignatures: vi.fn().mockRejectedValue(new Error("Indexer unavailable")),
      } as unknown as IndexerClient;
    ...
    it("should degrade to A alone when B times out", async () => {
      ...
      mockRouteTaskWithFallback.mockImplementationOnce(() =>
        new Promise((resolve) => setTimeout(() =>
          resolve({ provider: "deepseek", response: { content: "Late response" } }),
